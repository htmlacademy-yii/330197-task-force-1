...
<div class="site-index" style="display:block; min-height: 63vh">
    <?php
        $task = new Tasks();
        $query = $task->find()->select(['t.title','t.description','t.budget','t.dt_add','t.deadline','c.*'])->from('tasks t')->join('INNER JOIN', 'categories c', 't.idcategory = c.id')->orderBy(['c.id' => SORT_ASC])->limit(10);;
        $rows = $query->all();
    ?>
